...
func Test_queryKeeper_Enquire(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid", fields{Mapper}, args{sdkTypes.WrapSDKContext(Context), newQueryRequest(testFromID)}, newQueryResponse(keepers.QueryKeeper.(queryKeeper).mapper.NewCollection(types.WrapSDKContext(Context)).Fetch(key.NewKey(testFromID)).FetchRecord(key.NewKey(testFromID))), false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			queryKeeper := queryKeeper{
