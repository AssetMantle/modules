...
func Test_transactionKeeper_Transact(t *testing.T) {
	...
	keepers.ModifyKeeper.(transactionKeeper).mapper.NewCollection(types.WrapSDKContext(Context)).Add(record.NewRecord(testOrder))
	...
	tests := []struct {
		...
		{"valid", fields{Mapper, parameterManager, conformAuxiliary, supplementAuxiliary, transferAuxiliary, authenticateAuxiliary}, args{sdkTypes.WrapSDKContext(Context), NewMessage(fromAccAddress, testFromID, testOrderID, testRate, testRate, testHeight, mutableMetaProperties, mutableProperties).(*Message)}, newTransactionResponse(), false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			transactionKeeper := transactionKeeper{
