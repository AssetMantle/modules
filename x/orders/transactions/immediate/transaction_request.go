...
type transactionRequest struct {
	...
	MakerAssetID            string       `json:"makerAssetID" valid:"required~required field makerAssetID missing, matches(^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$)~invalid field makerAssetID"`
	TakerAssetID            string       `json:"takerAssetID" valid:"required~required field takerAssetID missing, matches(^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$)~invalid field takerAssetID"`
