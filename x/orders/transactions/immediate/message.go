...
func (message *Message) ValidateBasic() error {
	...
	if _, ok := sdkTypes.NewIntFromString(message.MakerOwnableSplit); !ok {
		return errorConstants.IncorrectFormat.Wrapf("maker ownable split %s is not a valid integer", message.MakerOwnableSplit)
	} else if _, ok := sdkTypes.NewIntFromString(message.TakerOwnableSplit); !ok {
		return errorConstants.IncorrectFormat.Wrapf("taker ownable split %s is not a valid integer", message.TakerOwnableSplit)
	}
	return nil
}
