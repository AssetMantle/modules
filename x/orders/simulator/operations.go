...
func simulateCancelMsg(module helpers.Module) simulationTypes.Operation {
	return func(rand *rand.Rand, baseApp *baseapp.BaseApp, context sdkTypes.Context, simulationAccountList []simulationTypes.Account, chainID string) (simulationTypes.OperationMsg, []simulationTypes.FutureOperation, error) {
		...
		makeMessage := make.NewMessage(from.Address, fromID, classificationID, toID, assetID.ToAnyOwnableID(), baseIDs.NewCoinID(baseIDs.NewStringID("stake")).ToAnyOwnableID(), baseTypesGo.NewHeight(-1), sdkTypes.NewInt(1), sdkTypes.NewInt(1), immutableMetaProperties, immutableProperties, mutableMetaProperties, mutableProperties)
		...
		orderID := baseIDs.NewOrderID(classificationID, baseQualified.NewImmutables(immutableMetaProperties.Add(utilities.AnyPropertyListToPropertyList(immutableProperties.GetList()...)...)))

		cancelMessage := cancel.NewMessage(from.Address, fromID, orderID)
...
func simulateTakeMsg(module helpers.Module) simulationTypes.Operation {
	return func(rand *rand.Rand, baseApp *baseapp.BaseApp, context sdkTypes.Context, simulationAccountList []simulationTypes.Account, chainID string) (simulationTypes.OperationMsg, []simulationTypes.FutureOperation, error) {
		...
		takeMessage := take.NewMessage(to.Address, toID, sdkTypes.NewInt(1), orderID)
