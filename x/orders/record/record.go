...
func NewRecord(order documents.Order) helpers.Record {
	return &Record{
		Key:      key.NewKey(baseIDs.NewOrderID(order.GetClassificationID(), order.GetImmutables())).(*key.Key),
		Mappable: mappable.NewMappable(order).(*mappable.Mappable),
	}
}
