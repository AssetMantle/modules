...
func Test_auxiliaryKeeper_Help(t *testing.T) {
	...
	keepers.ConformKeeper.(auxiliaryKeeper).mapper.NewCollection(sdkTypes.WrapSDKContext(Context)).Add(record.NewRecord(baseDocuments.NewClassification(immutables, mutables)))
	type fields struct {
	...
	tests := []struct {
		...
		{"entity not found", fields{Mapper}, args{sdkTypes.WrapSDKContext(Context), NewAuxiliaryRequest(testClassificationID1, immutables1, mutables1)}, nil, true},
		{"valid", fields{Mapper}, args{sdkTypes.WrapSDKContext(Context), NewAuxiliaryRequest(classificationID, immutables, mutables)}, newAuxiliaryResponse(), false},
		{"incorrect format", fields{Mapper}, args{sdkTypes.WrapSDKContext(Context), NewAuxiliaryRequest(classificationID, immutables1, mutables)}, nil, true},
	}
