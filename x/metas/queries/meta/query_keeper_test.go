...
func createTestInput(t *testing.T) (sdkTypes.Context, TestKeepers, helpers.Mapper, helpers.ParameterManager) {
	...
	keepers := TestKeepers{
		MetaKeeper: keeperPrototype().Initialize(Mapper, parameterManager, []interface{}{}).(helpers.QueryKeeper),
	}
...
func Test_queryKeeper_Enquire(t *testing.T) {
	...
	testDataID := baseIDs.GenerateDataID(baseData.NewStringData("Data"))
	keepers.MetaKeeper.(queryKeeper).mapper.NewCollection(sdkTypes.WrapSDKContext(Context)).Add(record.NewRecord(baseData.NewStringData("Data")))
	...
	tests := []struct {
		...
		{"valid", fields{Mapper}, args{sdkTypes.WrapSDKContext(Context), newQueryRequest(testDataID)}, newQueryResponse(keepers.MetaKeeper.(queryKeeper).mapper.NewCollection(sdkTypes.WrapSDKContext(Context)).Fetch(newQueryRequest(testDataID).(*QueryRequest).Key).FetchRecord(newQueryRequest(testDataID).(*QueryRequest).Key)), false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			queryKeeper := queryKeeper{
