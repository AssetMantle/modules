...
func Test_queryKeeper_Enquire(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid", fields{Mapper}, args{sdkTypes.WrapSDKContext(Context), newQueryRequest(splitID)}, newQueryResponse(record.NewRecord(baseIDs.NewSplitID(testAssetID, testOwnerIdentityID), split)), false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			queryKeeper := queryKeeper{
