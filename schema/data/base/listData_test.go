...
func TestListDataPrototype(t *testing.T) {
	dataList := NewListData(NewStringData("mukund"), NewStringData("prabhav"))
	aman := NewStringData("aman")

	_, found := dataList.Search(aman)

	if !found {
		dataList = dataList.Add(aman)
		_, found = dataList.Search(aman)
		if found {
			dataList = dataList.Remove(aman)
		}
		dataList.Search(aman)
	}
}
