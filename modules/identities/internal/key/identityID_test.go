...
func Test_IdentityID_Methods(t *testing.T) {
	...
	testIdentityID := NewIdentityID(classificationID, base.NewImmutables(immutableProperties))
	testIdentityID2 := NewIdentityID(classificationID, base.NewImmutables(emptyImmutableProperties))
	key := FromID(testIdentityID)

	require.Equal(t, testIdentityID, identityID{ClassificationID: classificationID, HashID: base.NewImmutables(immutableProperties).GenerateHashID()})
	...
	require.Equal(t, testIdentityID.(identityID).Bytes(), append(classificationID.Bytes(), base.NewImmutables(immutableProperties).GenerateHashID().Bytes()...))
