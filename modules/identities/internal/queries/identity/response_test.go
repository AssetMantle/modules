	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			queryResponse := &QueryResponse{
				...
				List:    mappable.ProduceList(tt.fields.List),
			}
...
func Test_queryResponse_Encode(t *testing.T) {
	context := CreateTestInputContext(t)
	collection := mapper.Prototype().NewCollection(context)
	encodedByte, err := common.LegacyAmino.MarshalJSON(&QueryResponse{Success: true, Error: "", List: mappable.ProduceList(collection.GetList())})
	encodedByteWithError, _err := common.LegacyAmino.MarshalJSON(&QueryResponse{Success: false, Error: constants.IncorrectFormat.Error(), List: mappable.ProduceList(collection.GetList())})
	require.Nil(t, err)
	type fields struct {
		Success bool
		Error   error
