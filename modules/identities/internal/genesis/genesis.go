...
func (genesis Genesis) Initialize(mappableList []helpers.Mappable, parameterList []types.Parameter) helpers.Genesis {
	...
	newMappableList := make([]mappable.Identity, len(mappableList))
	for i, _ := range mappableList {
		newMappableList[i] = *mappableList[i].(*mappable.Identity)
	}
...
func NewGenesis(defaultMappableList []mappable.Identity, defaultParameterList []dummy.DummyParameter) *Genesis {
	return &Genesis{
		DefaultMappableList:  defaultMappableList,
		DefaultParameterList: defaultParameterList,
		MappableList:         []mappable.Identity{},
