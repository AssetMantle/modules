...
func (auxiliaryKeeper auxiliaryKeeper) Help(context sdkTypes.Context, AuxiliaryRequest helpers.AuxiliaryRequest) helpers.AuxiliaryResponse {
	...
	if auxiliaryResponse := auxiliaryKeeper.splitsBurnAuxiliary.GetKeeper().Help(context, burn.NewAuxiliaryRequest(auxiliaryRequest.MakerID, auxiliaryRequest.MakerSplitID, auxiliaryRequest.MakerSplit)); !auxiliaryResponse.IsSuccessful() {
		return newAuxiliaryResponse(auxiliaryResponse.GetError())
	}
	if auxiliaryResponse := auxiliaryKeeper.splitsMintAuxiliary.GetKeeper().Help(context, mint.NewAuxiliaryRequest(base.NewID(mapper.ModuleName), auxiliaryRequest.MakerSplitID, auxiliaryRequest.MakerSplit)); !auxiliaryResponse.IsSuccessful() {
		return newAuxiliaryResponse(auxiliaryResponse.GetError())
	}
