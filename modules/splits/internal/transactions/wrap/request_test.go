...
func Test_Unwrap_Request(t *testing.T) {
	...
	msg2, error2 := newTransactionRequest(rest.BaseReq{From: "randomFromAddrs", ChainID: "test"}, "fromID", "2 stake").MakeMsg()
	require.NotNil(t, error2)
	require.Nil(t, msg2)

	msg2, error2 = newTransactionRequest(testBaseReq, "fromID", "randomString").MakeMsg()
	require.NotNil(t, error2)
	require.Nil(t, msg2)
