...
func (genesis Genesis) Initialize(mappableList []helpers.Mappable, parameterList []types.Parameter) helpers.Genesis {
	...
	newMappableList := make([]mappable.Split, len(mappableList))
	for i, _ := range mappableList {
		newMappableList[i] = *mappableList[i].(*mappable.Split)
	}
...
func NewGenesis(defaultMappableList []mappable.Split, defaultParameterList []dummy.DummyParameter) *Genesis {
	return &Genesis{
		DefaultMappableList:  defaultMappableList,
		DefaultParameterList: defaultParameterList,
		MappableList:         []mappable.Split{},
