...
func (genesis Genesis) Initialize(mappableList []helpers.Mappable, parameterList []types.Parameter) helpers.Genesis {
	...
	newMappableList := make([]mappable.Order, len(mappableList))
	for i, _ := range mappableList {
		newMappableList[i] = *mappableList[i].(*mappable.Order)
	}
...
func NewGenesis(defaultMappableList []mappable.Order, defaultParameterList []dummy.DummyParameter) *Genesis {
	return &Genesis{
		DefaultMappableList:  defaultMappableList,
		DefaultParameterList: defaultParameterList,
		MappableList:         []mappable.Order{},
