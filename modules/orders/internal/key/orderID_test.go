...
func Test_OrderID_Methods(t *testing.T) {
	...
	require.Equal(t, testOrderID, orderID{ClassificationID: classificationID, MakerOwnableID: makerOwnableID, TakerOwnableID: takerOwnableID, RateID: rateID, CreationID: creationID, MakerID: makerID, HashID: baseTraits.Immutables{PropertyList: immutableProperties}.GenerateHashID()})
	require.Equal(t, true, testOrderID.Equals(testOrderID))
	require.Equal(t, false, testOrderID.Compare(baseIDs.NewID("")) == 0)
	require.Equal(t, strings.Join([]string{classificationID.String(), makerOwnableID.String(), takerOwnableID.String(), rateID.String(), creationID.String(), makerID.String(), baseTraits.Immutables{PropertyList: defaultImmutableProperties}.GenerateHashID().String()}, constants.SecondOrderCompositeIDSeparator), testOrderID.String())
	...
	require.Equal(t, testOrderID, FromID(baseIDs.NewID(classificationID.String()+constants.SecondOrderCompositeIDSeparator+makerOwnableID.String()+constants.SecondOrderCompositeIDSeparator+takerOwnableID.String()+constants.SecondOrderCompositeIDSeparator+rateID.String()+constants.SecondOrderCompositeIDSeparator+creationID.String()+constants.SecondOrderCompositeIDSeparator+makerID.String()+constants.SecondOrderCompositeIDSeparator+baseTraits.Immutables{PropertyList: defaultImmutableProperties}.GenerateHashID().String())))
