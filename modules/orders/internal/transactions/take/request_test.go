...
func Test_Take_Request(t *testing.T) {
	...
	require.Equal(t, newMessage(fromAccAddress, baseIDs.NewStringID("fromID"), sdkTypes.NewDec(3), baseIDs.NewStringID("orderID")), msg)
	require.Nil(t, err)

	msg2, err := newTransactionRequest(rest.BaseReq{From: "randomFromAddress", ChainID: "test"}, "fromID", "3", "orderID").MakeMsg()
	require.NotNil(t, err)
	require.Nil(t, msg2)

	msg2, err = newTransactionRequest(testBaseReq, "fromID", "randomString", "orderID").MakeMsg()
	require.NotNil(t, err)
	require.Nil(t, msg2)
