...
func (transactionKeeper transactionKeeper) Transact(context sdkTypes.Context, msg sdkTypes.Msg) helpers.TransactionResponse {
	...
	if order != nil {
		order = mappable.NewOrder(orderID, immutableProperties, order.(mappables.Order).GetMutableProperties().Mutate(mutableProperties.GetList()...))
		orders = orders.Mutate(order)
	} else {
		order = mappable.NewOrder(orderID, immutableProperties, mutableProperties).(mappables.Order)
	...
	orders.Iterate(key.FromID(key.NewOrderID(order.(mappables.Order).GetClassificationID(), order.(mappables.Order).GetTakerOwnableID(), order.(mappables.Order).GetMakerOwnableID(), base.NewID(""), base.NewID(""), base.NewID(""), base.NewProperties())), accumulator)
	...
	if !orderLeftOverMakerOwnableSplit.Equal(sdkTypes.ZeroDec()) && orderMutated {
		...
		orders.Mutate(mappable.NewOrder(orderID, order.(mappables.Order).GetImmutableProperties(), order.(mappables.Order).GetMutableProperties().Mutate(mutableProperties.GetList()...)))
	}
