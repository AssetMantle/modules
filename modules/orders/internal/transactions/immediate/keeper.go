...
func (transactionKeeper transactionKeeper) Transact(context sdkTypes.Context, msg sdkTypes.Msg) helpers.TransactionResponse {
	...
	mutableMetaProperties := message.MutableMetaProperties.Add(base.NewMetaProperty(ids.Expiry, base.NewMetaFact(base.NewHeightData(base.NewHeight(message.ExpiresIn.Get()+context.BlockHeight())))))
	mutableMetaProperties = mutableMetaProperties.Add(base.NewMetaProperty(ids.MakerOwnableSplit, base.NewMetaFact(base.NewDecData(message.MakerOwnableSplit))))
	...
	accumulator := func(mappableOrder helpers.Mappable) bool {
		...
		if makerOwnableSplitProperty := executableOrderMetaProperties.Get(ids.MakerOwnableSplit); makerOwnableSplitProperty != nil {
		...
		if orderExchangeRate.MulTruncate(executableOrderExchangeRate).MulTruncate(sdkTypes.SmallestDec()).MulTruncate(sdkTypes.SmallestDec()).LTE(sdkTypes.OneDec()) {
			...
			case orderLeftOverMakerOwnableSplit.LT(executableOrderTakerOwnableSplitDemanded):
				...
				mutableProperties, Error := scrub.GetPropertiesFromResponse(transactionKeeper.scrubAuxiliary.GetKeeper().Help(context, scrub.NewAuxiliaryRequest(base.NewMetaProperty(ids.MakerOwnableSplit, base.NewMetaFact(base.NewDecData(executableOrderMakerOwnableSplit.Sub(sendToBuyer)))))))
				if Error != nil {
					panic(Error)
				}
	...
	if !orderLeftOverMakerOwnableSplit.Equal(sdkTypes.ZeroDec()) && orderMutated {
		mutableProperties, Error := scrub.GetPropertiesFromResponse(transactionKeeper.scrubAuxiliary.GetKeeper().Help(context, scrub.NewAuxiliaryRequest(base.NewMetaProperty(ids.MakerOwnableSplit, base.NewMetaFact(base.NewDecData(orderLeftOverMakerOwnableSplit))))))
		if Error != nil {
			return newTransactionResponse(Error)
		}
