...
func (genesis Genesis) Initialize(mappableList []helpers.Mappable, parameterList []types.Parameter) helpers.Genesis {
	...
	newMappableList := make([]mappable.Maintainer, len(mappableList))
	for i, _ := range mappableList {
		newMappableList[i] = *mappableList[i].(*mappable.Maintainer)
	}
...
func NewGenesis(defaultMappableList []mappable.Maintainer, defaultParameterList []dummy.DummyParameter) *Genesis {
	return &Genesis{
		DefaultMappableList:  defaultMappableList,
		DefaultParameterList: defaultParameterList,
		MappableList:         []mappable.Maintainer{},
