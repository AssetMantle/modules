...
Once a Persistence-SDK release has been completed and published, updates for it are released under certain circumstances.

### Rationale

Unlike in-development `master` branch snapshots, **Persistence-SDK** releases are subject to much wider adoption,
...
* **High-impact bugs**
    ...
    * *Severe regressions* from a Persistence-SDK's previous release. This includes all sort of issues
...
1. Check that the bug is either fixed or not reproducible in `master`. It is, in general, not appropriate to release bug fixes for stable releases without first testing them in `master`. Please apply the label [v0.43](https://github.com/persistenceone/persistence-sdk/milestone/26) to the issue.
...
The **Stable Release Managers** evaluate and approve or reject updates and backports to Persistence-SDK Stable Release series,
according to the [stable release policy](#stable-release-policy).
